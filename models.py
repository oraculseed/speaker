...
class AT_net(nn.Module):
    ...
    def forward(self, example_landmark, audio):
        hidden = ( torch.autograd.Variable(torch.zeros(3, audio.size(0), 256)),
                      torch.autograd.Variable(torch.zeros(3, audio.size(0), 256)))
...
class GL_Discriminator(nn.Module):
    ...
    def forward(self, xs, example_landmark):
        hidden = ( torch.autograd.Variable(torch.zeros(3, example_landmark.size(0), 256)),
                      torch.autograd.Variable(torch.zeros(3, example_landmark.size(0), 256)))
        lstm_input = list()
